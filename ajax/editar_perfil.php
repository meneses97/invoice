<?php
	...
	if (empty($_POST['nombre_empresa'])) {
           $errors[] = "Nome  da Empresa vazio";
        }else if (empty($_POST['telefono'])) {
           $errors[] = "Telefone vazio";
        } else if (empty($_POST['email'])) {
           $errors[] = "E-mail esta vaczio";
//        } else if ($_POST['impuesto']!="ISENTO" || $_POST['impuesto'] > 0 ) {
//           $errors[] = "IVA esta vazio";
        } else if (empty($_POST['moneda'])) {
           $errors[] = "Moeda Vazio";
        } else if (empty($_POST['direccion'])) {
           $errors[] = "Endereço Vazio";
...
//			!empty($_POST['impuesto']) &&
        ...
        $conta=mysqli_real_escape_string($con,(strip_tags($_POST["conta"],ENT_QUOTES)));
        $nib=mysqli_real_escape_string($con,(strip_tags($_POST["nib"],ENT_QUOTES)));
		...
		$sql="UPDATE perfil SET nombre_empresa='".$nombre_empresa."', telefono='".$telefono."', email='".$email."',
		 impuesto='".$impuesto."', moneda='".$moneda."', direccion='".$direccion."',
		  ciudad='".$ciudad."', estado='".$estado."', codigo_postal='".$codigo_postal."',
		  conta='".$conta."', nib='".$nib."', banco='".$banco."' WHERE id_perfil='1'";
		$query_update = mysqli_query($con,$sql);
			if ($query_update){
				$messages[] = "Dados actualizados com sucesso";
			} else{
				$errors []= "A problemas nos dados fornecidos".mysqli_error($con);
			}
		} else {
