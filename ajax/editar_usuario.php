...
if (version_compare(PHP_VERSION, '5.3.7', '<')) {
    ...
    require_once("../libraries/password_compatibility_library.php");
		...
		if (empty($_POST['firstname2'])){
            ...
            $errors[] = "Nome do utilizador vazio";
        }  elseif (strlen($_POST['user_name2']) > 64 || strlen($_POST['user_name2']) < 2) {
            $errors[] = "Nome do utilizador não pode ser inferiro a 2 o mais de 64 caracteres";
        } elseif (!preg_match('/^[a-z\d]{2,64}$/i', $_POST['user_name2'])) {
            $errors[] = "Nome do utilizador so deve ter: letras de aZ e numeros permitidos , de 2 a 64 caracteres";
        } elseif (empty($_POST['user_email2'])) {
            $errors[] = "O correio electrónico não puede estar vazío";
        } elseif (strlen($_POST['user_email2']) > 64) {
            $errors[] = "O correo electrónico não pode ser superior a 64 caracteres";
        } elseif (!filter_var($_POST['user_email2'], FILTER_VALIDATE_EMAIL)) {
            $errors[] = "Seu endereço de correio electronico não esta é um formato valido";
        } elseif (
			...
			require_once ("../config/conexion.php");//Contiene funcion que conecta a la base de datos
             ...
             $user_id=intval($_POST['mod_id']);
                    ...
                    $sql = "UPDATE users SET firstname='".$firstname."', lastname='".$lastname."',
                     user_name='".$user_name."', user_email='".$user_email."', role='".$user_role."'
                            WHERE user_id='".$user_id."';";
                    $query_update = mysqli_query($con,$sql);

                    // if user has been added successfully
                    if ($query_update) {
                        $messages[] = "A conta foi modificada com sucesso";
                    } else {
                        $errors[] = "A problemas no registo tente novamente";
                    }
                
            
        } else {
            $errors[] = "Um erro desconhecido ocorreu.";
        }
